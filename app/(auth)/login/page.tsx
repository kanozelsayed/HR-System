...
function LoginContent() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (email === "admin@smarthire.ai" && password === "password123") {
        ...
        // Extracting name from email or set a custom name
        const displayName = email.split('@')[0]; // will take 'admin'

        setTimeout(() => {
          // Sending the name to dashboard
          router.push(`/dashboard?name=${displayName}`);
        }, 1000);
      } else {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4 py-12 relative font-sans">
      <button onClick={() => router.push("/")} className="absolute top-8 left-8 flex items-center gap-2 text-gray-500 hover:text-blue-600 transition-all font-medium text-sm group">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <div className="relative">
              <input type={showPassword ? "text" : "password"} placeholder="••••••••" onBlur={() => handleBlur("password")} className={`w-full px-4 py-2.5 border rounded-xl outline-none transition-all ${touched.password && errors.password ? "border-red-500 bg-red-50/30" : "border-gray-200 focus:border-blue-500"}`} value={password} onChange={(e) => setPassword(e.target.value)} />
              <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 text-xs font-bold uppercase">{showPassword ? "Hide" : "Show"}</button>
            </div>
            {touched.password && errors.password && <p className="text-red-500 text-[10px] mt-1 ml-2">{errors.password}</p>}
          </div>
          <button type="submit" disabled={!isValid || isSubmitting} className={`w-full py-3 rounded-2xl font-bold text-sm transition-all ${isValid && !isSubmitting ? "bg-blue-600 text-white shadow-lg" : "bg-gray-100 text-gray-400 cursor-not-allowed"}`}>
            {isSubmitting ? "Processing..." : "Login"}
          </button>
        </form>
        <p className="text-center mt-8 text-sm text-gray-500">Don&apos;t have an account? <Link href="/signup" className="text-blue-600 font-bold hover:underline underline-offset-4">Sign up</Link></p>
      </div>
      {toast && (
        <div className={`fixed top-10 right-10 z-[100] flex items-center px-6 py-3 rounded-2xl shadow-2xl border text-white transition-all duration-500 ${toast.type === "success" ? "bg-emerald-500 border-emerald-400" : "bg-red-500 border-red-400"}`}>
