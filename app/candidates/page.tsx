...
function ScoreProgressBar({ score }: { score: number }) {
  const getColor = () => score >= 85 ? "bg-emerald-500" : score >= 65 ? "bg-amber-400" : "bg-red-400";
  const getBadgeStyle = () => score >= 85 ? "text-emerald-600 bg-emerald-50 border-emerald-100" : score >= 65 ? "text-amber-600 bg-amber-50 border-amber-100" : "text-red-600 bg-red-50 border-red-100";

  return (
    <div className="flex items-center gap-2 w-full max-w-[180px]">
      <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
        <div className={`h-full rounded-full transition-all duration-700 ${getColor()}`} style={{ width: `${score}%` }} />
      </div>
      <span className={`px-2 py-0.5 rounded-full border text-xs font-black whitespace-nowrap ${getBadgeStyle()}`}>
        {score}%
      </span>
    </div>
  );
...
export default function CandidateListPage() {
  ...
  const getScoreStyle = (score: number) => score >= 85 ? "text-emerald-600 bg-emerald-50 border-emerald-100" : score >= 65 ? "text-amber-600 bg-amber-50 border-amber-100" : "text-red-600 bg-red-50 border-red-100";

  if (authLoading) return (
    <div className="p-10 text-center text-slate-500 font-medium">Loading SmartHire AI...</div>
  );

  return (
    <div className="p-4 sm:p-6 lg:p-10 max-w-6xl mx-auto bg-white min-h-screen">

      {/* Header */}
      <header className="mb-6 sm:mb-10 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight">Candidate Ranking</h1>
          <p className="text-slate-500 text-sm mt-1">AI-driven analysis for {MOCK_CANDIDATES.length} applicants.</p>
        </div>
        <div className="bg-blue-50 text-blue-700 px-4 py-2 rounded-xl text-sm font-bold border border-blue-100 self-start sm:self-auto">
          AI Match Scores
        </div>
      </header>
          ...
          <tbody className="divide-y divide-slate-100">
            {MOCK_CANDIDATES.map((c) => (
              <tr key={c.id} className="hover:bg-slate-50/80 transition-all group">
                ...
                <td className="px-6 py-4"><ScoreProgressBar score={c.score} /></td>
                <td className="px-6 py-4 text-right">
                  <button onClick={() => { setSelectedCandidate(c); setIsModalOpen(true); }}
        ...
        {MOCK_CANDIDATES.map((c) => (
          <div key={c.id} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
            <div className="flex justify-between items-start mb-3">
              ...
              <div className={`px-2.5 py-1 rounded-full border text-xs font-black ${getScoreStyle(c.score)}`}>
                {c.score}%
              </div>
            </div>
            ...
            <button
              ...
              className="mt-3 w-full py-2 rounded-xl border border-blue-100 text-blue-600 text-sm font-bold hover:bg-blue-50 transition-all">
              View Feedback
            </button>
          </div>
        ))}
      ...
      {isModalOpen && selectedCandidate && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
          <div className="bg-white w-full sm:max-w-lg rounded-t-[2rem] sm:rounded-[2rem] shadow-2xl overflow-hidden animate-in slide-in-from-bottom sm:zoom-in duration-300">

            {/* Modal Header */}
            <div className="px-6 py-4 flex justify-between items-center border-b border-slate-100 bg-slate-50/50">
              <div>
                <h3 className="text-lg font-black text-slate-900">AI Feedback</h3>
                <p className="text-xs text-blue-600 font-bold uppercase mt-0.5">{selectedCandidate.name}</p>
              </div>
              <button onClick={() => setIsModalOpen(false)}
                className="text-slate-400 hover:text-slate-900 p-1.5 bg-white rounded-full shadow-sm">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="bg-blue-600 px-5 py-4 rounded-2xl shadow-lg shadow-blue-100">
                <h5 className="text-blue-100 font-bold text-[9px] uppercase tracking-widest mb-1">AI Recommendation</h5>
                <p className="text-white text-xs font-medium leading-relaxed">"{selectedCandidate.feedback.recommendation}"</p>
              </div>
            </div>
